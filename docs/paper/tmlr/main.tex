...
structure by a potential-term ablation (all five per-seed paired differences
positive; across-seed sign test $p=0.031$). This pass exists only under the
revised protocol, so we test it further: a pre-registered out-of-sample
replication on a fresh, previously unused seed family (the P2 pipeline frozen
verbatim) reproduces the pass on four of five seeds, raising it from a
revised-protocol result to a revised-architecture-plus-fresh-confirmation one.
The apparent affine
